 ...
 * ממיר כל ספרה למילה עברית עם נקודה אחריה — יוצר הפסקה טבעית בין כל ספרה.
 * לדוגמה: "054" → "אפס. חמש. ארבע."
 */
function formatPhoneForTts(phone) {
    if (!phone) return '';
    const map = { '0':'אפס', '1':'אחת', '2':'שתיים', '3':'שלוש', '4':'ארבע',
                  '5':'חמש',  '6':'שש',   '7':'שבע',   '8':'שמונה', '9':'תשע' };
    return String(phone).replace(/\D/g, '').split('').map(d => map[d] || d).join('. ') + '.';
}
